...
class Program
{
    ...
    static void WriteDuplicatesToSheet(IXLWorksheet sheet, List<Duplicate> duplicates, string title)
    {
        ...
        foreach (var item in duplicates)
        {
            ...
            // Add first file name in bold below the fragment
            sheet.Cell(row, 2).Value = $"First File: {item.FirstFile?.Name}";
            sheet.Cell(row, 2).Style.Font.Bold = true;
            row++;

            // Add second file name in bold below the first file name
            sheet.Cell(row, 2).Value = $"Last File: {item.SecondFile?.Name}";
            sheet.Cell(row, 2).Style.Font.Bold = true;
            row++;
